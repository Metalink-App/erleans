...
call(GrainRef, Request, Timeout) ->
    ...
    do_for_ref(GrainRef, fun(_, Pid) ->
                                 try
                                     gen_statem:call(Pid, {ot_tracer:current_span_ctx(), ReqType, Request}, Timeout)
                                 catch
                                     exit:{bad_etag, _} ->
...
cast(GrainRef, Request) ->
    ReqType = req_type(),
    do_for_ref(GrainRef, fun(_, Pid) -> gen_statem:cast(Pid, {ot_tracer:current_span_ctx(), ReqType, Request}) end).
...
active({call, From}, {SpanCtx, ReqType, Msg}, Data=#data{cb_module=CbModule,
                                                         cb_state=CbData,
                                                         deactivate_after=DeactivateAfter}) ->
    ot_tracer:start_span(opentelemetry:get_application_tracer(?MODULE),span_name(Msg), #{parent => SpanCtx}),
    otel_span:set_attribute(ot_tracer:current_span_ctx(), <<"grain_msg">>, io_lib:format("~p", [Msg])),
    try handle_result(CbModule:handle_call(Msg, From, CbData), Data, upd_timer(ReqType, DeactivateAfter))
    after
        otel_span:end_span(ot_tracer:current_span_ctx(), opentelemetry:timestamp())
    end;
